(function() {
  ...
  //Object.keys(...).forEach is not a function
...
//should return empty array if zero is passed in as the index 
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(value, x){
  ...
  //should cast the result to a boolean 
  ...
  //_.extend(ted, {hair:black, age:23})
  ...
  _.memoize = function(func) {
    ...
    return function(){
      ...
      //go through each key in cache
      for(var key in cache){
        //if matches (means it is already in cache, then just pull out the value from cache)
        if(key == arg){
          return cache[key];
        }
      }
      ...
      //if not in cache
      return cache[arg] = func.apply(this,arg);
    };
  };
